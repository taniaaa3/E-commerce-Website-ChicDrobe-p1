let hoodies = [];
let func = async () => {
    const res = await fetch('../js/men/men.json');
    const data = await res.json();
    hoodies = await data.filter((val) => {
        return val.type == "Hoodies";
    })


    let male = document.querySelector(".hoodies");
    male.innerHTML = hoodies.map((val) => {
        return (`
